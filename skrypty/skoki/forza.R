...
fps_data <- data.frame(
  fps_base = read.csv(file = './dane/forza/bazowy.csv', sep = ';')$Framerate,
  fps_dlss_quality = read.csv(file = './dane/forza/dlss-jakosc.csv', sep = ';')$Framerate,
  fps_dlss_balance = read.csv(file = './dane/forza/dlss-balans.csv', sep = ';')$Framerate,
  fps_dlss_performance = read.csv(file = './dane/forza/dlss-wydajnosc.csv', sep = ';')$Framerate,
  fps_fsr_quality = read.csv(file = './dane/forza/fsr-jakosc.csv', sep = ';')$Framerate,
  fps_fsr_balance = read.csv(file = './dane/forza/fsr-balans.csv', sep = ';')$Framerate,
  fps_fsr_performance = read.csv(file = './dane/forza/fsr-wydajnosc.csv', sep = ';')$Framerate
)
...
ggplot(fps_data_diff_tidy, aes(x = id, y = fps_diff, color = setting)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Frame Rate Variability Plot (Forza)", x = "Time", y = "Variability", color = "Settings") +
  scale_color_discrete(labels = new_legend_labels) +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(breaks = seq(0, 100, by = 1), 
                     labels = ifelse(seq(0, 100, by = 1) %% 10 == 0, seq(0, 100, by = 1), ""),
                     limits = c(0, 100)) +
