...
ggplot(fps_data_diff_tidy, aes(x = id, y = fps_diff, color = setting)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Frame Rate Variability Plot (Red Dead Redemption 2)", x = "Time", y = "Variability", color = "Settings") +
  scale_color_discrete(labels = new_legend_labels) +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_x_continuous(breaks = seq(0, 232, by = 1), 
                     labels = ifelse(seq(0, 232, by = 1) %% 10 == 0, seq(0, 232, by = 1), ""),
                     limits = c(0, 232)) +
